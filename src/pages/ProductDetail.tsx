...
const ProductDetail: React.FC = () => {
  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <Link to="/" className="bg-black text-white px-6 py-2 hover:bg-gray-800 transition-colors">Back to Home</Link>
      </div>
    );
  }
  ...
  return (
    <>
    <div className="bg-[#f9f2e8] px-40">
      {/* Breadcrumbs */}
      <nav className="container mx-auto px-4 pt-8 pb-4 flex items-center text-sm text-gray-600">
        <Link to="/" className="hover:text-black transition-colors">Home</Link>
        <ChevronRight size={16} className="mx-2" />
        <Link to={`/collection/${product.category.toLowerCase()}`} className="hover:text-black transition-colors capitalize">
          {product.category}
        </Link>
        <ChevronRight size={16} className="mx-2" />
      ...
      <div className="container mx-auto px-4 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
              ...
              {product.images.map((image: string, index: number) => (
                <button 
                  ...
                  className={`aspect-square overflow-hidden rounded-md border-2 transition-all ${
                    selectedImage === image ? 'border-black' : 'border-transparent opacity-70 hover:opacity-100'
                  }`}
          ...
          <div className="space-y-6">
            ...
            <div>
              <h2 className="text-5xl font-light text-gray-800 mb-2">{product.name}</h2>
              <h1 className="text-lg font-medium text-black uppercase tracking-wide">Description Of This Product</h1>
            </div>

            {/* Price */}
            <div>
              <p className="text-lg text-black">Rs. {product.price.toLocaleString()}.00</p>
            ...
            <div className="space-y-0 border-t border-gray-200 pt-6">
              ...
              <div className="border-b border-black">
                <button
                  ...
                  className="w-full flex items-center justify-between py-4 text-left"
                >
                  <span className="text-lg text-black">Delivery Timelines</span>
              ...
              <div className="border-b border-black">
                ...
                >
                  ...
                  {expandedSections.disclaimer ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </button>
                {expandedSections.disclaimer && (
                  <div className="pb-4">
                    <p className="text-sm text-gray-600 leading-relaxed">
                      Product colors may vary slightly due to photographic lighting or your monitor settings. 
                      Handcrafted products may have minor variations that make each piece unique.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="border-b border-black">
                <button
                  ...
                  className="w-full flex items-center justify-between py-4 text-left"
                >
                  <span className="text-lg text-black">Washcare Instructions</span>
                  {expandedSections.washcare ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </button>
